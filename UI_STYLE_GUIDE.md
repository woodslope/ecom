...
- Feedback ownership follows the task scope: action-level waiting stays on the triggering Button; a cross-workspace operation with cancellation or continued work uses one global `Toast` in `ToastRegion`; a local lock reason stays beside the affected control as `StatusMessage` or `OperationStatus`. The same state must not be rendered as both a global Toast and a page block. Button loading may remain as the local repeat-submit guard, but it must not repeat the full task description.
- Planning, generation, cancellation, and Copilot progress share the `OperationStatus` content pattern (icon, title, description, and actions), rendered through a global Toast when the operation can outlive the current view. `OperationStatus` remains the local block primitive for scoped locks and preparation surfaces; it does not imply a full-width placement.
...
- Batch generation keeps detailed progress, resume, retry, and cancel controls in the on-demand history task panel; while an item is actively generating, the global generation Toast is the single live progress surface. Taobao analysis uses the local AI策划 Button loading state and hands off to the platform-planning Toast once planning starts.
