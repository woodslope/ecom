...
import { ArchiveRestore, ChevronDown, CopyPlus, Download, RotateCcw, Smartphone } from "lucide-react";
...
export function ProductionRunCard({ record, expanded, current, assetUrls, busy, onToggle, onResume, onFork, onReuse, onExport, onDeposit }: {
  ...
  return (
    <article className={`production-run-card${current ? " production-run-card--current" : ""}`}>
      <button type="button" className="production-run-card__summary" aria-expanded={expanded} onClick={onToggle}>
        ...
        <span className="production-run-card__chips"><StatusChip tone={run.status === "ready" ? "success" : run.status === "failed" ? "warning" : "info"}>{statusLabels[run.status]}</StatusChip><StatusChip tone="neutral">{run.source === "api" ? "API" : "Demo"}</StatusChip><StatusChip tone={manual ? "warning" : "info"}>{manual ? "手动来源" : "资料库来源"}</StatusChip>{current ? <StatusChip tone="success">当前任务</StatusChip> : null}</span>
      ...
      {expanded ? <div className="production-run-card__body">
        <div className="production-run-card__actions">
          ...
          {manual && onDeposit ? <Button variant="secondary" size="compact" disabled={busy} onClick={onDeposit}><ArchiveRestore size={14} />沉淀到资料库</Button> : null}
        ...
        <div className="production-run-card__facts"><span>{run.planSnapshot.slots.length} 个槽位</span><span>{outputs.length} 张输出</span><span>最近：{lastEvent ? eventLabels[lastEvent.kind] : "无事件"}</span>{!manual ? <span>来源商品：{record.sourceProject?.name ?? "已删除商品"}</span> : null}{run.deposit ? <span>已沉淀：{new Date(run.deposit.depositedAt).toLocaleString("zh-CN")}</span> : null}</div>
