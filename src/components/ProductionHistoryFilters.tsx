...
export function ProductionHistoryFilters({ value, onChange, onClear, hidePlatform = false, compact = false }: {
  ...
  return (
    <div className={`production-history-filters${compact ? " production-history-filters--compact" : ""}`} aria-label="生产记录筛选">
      ...
      {!compact ? <Field label="来源" name="source"><Select value={value.source ?? ""} onChange={(event) => set("source", event.target.value as ProductionRunFilters["source"])}><option value="">全部</option><option value="demo">Demo</option><option value="api">API</option></Select></Field> : null}
